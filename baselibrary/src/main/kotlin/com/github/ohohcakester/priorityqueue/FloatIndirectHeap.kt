 ...
 */
class FloatIndirectHeap(private val minHeap: Boolean) : Serializable {
	private val keyList = ArrayList<Float>()
	private val inList = ArrayList<Int>()
	private val outList = ArrayList<Int>()

	private var comparator: Comparator<Float>? = null

	/**
	 * Runtime: O(n)
	 */
	constructor(array: FloatArray, minHeap: Boolean) : this(minHeap) {
	...
	private fun compare(a: Float, b: Float) = when (comparator) {
		null -> a.compareTo(b)
	...
	fun setComparator(comparator: Comparator<Float>) {
	 ...
	 */
	private fun swapE(list: ArrayList<Float>, i1: Int, i2: Int) {
		val temp = list[i1]
		list[i1] = list[i2]
		list[i2] = temp
	}
